...
function openModal(category){
    ...
    categories[category].forEach(nominee => {
        ...
        div.innerHTML = `<img src="${nominee.img}"><span>${nominee.name}</span><button>Выбрать</button>`;
    ...
    // Прокручиваем окно к верху страницы
...
confirmVoteBtn.addEventListener("click", ()=> {
  ...
  // Сохраняем локально (уже есть у тебя)
  ...
  // Здесь вставляем отправку на сервер
  fetch("http://localhost:3000/vote", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ category: selectedCategory, nominee: selectedNominee })
  })
  .then(res => res.json())
  .then(data => {
    if(data.success) {
      alert("Голос сохранён на сервере!");
    }
  });
...
function updateVoteDisplay(category){
    ...
    sel.textContent = `Вы выбрали: ${votes[category]}`;
}
// При загрузке страницы подставляем ранее сохранённые голоса
